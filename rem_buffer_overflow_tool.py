...
def getJmpESPAddr(badbytes, args):
...
def crashcheck(target, args):
    if args.no_crashcheck:
        print('Assuming target down.')
        return True
...
def parse_args():
    ...
    parser.add_argument('--pattern-type', dest='pattern_type'
                        , help='Type of pattern. either MSF or A'
                        , default='MSF'
                        , choices=['MSF', 'A'])
    ...
    parser.add_argument('--no-crashcheck', dest='no_crashcheck',
                        help='Assume target crashed, don\'t check.', choices=['True', 'False'], default='False')
    ...
    args.no_crashcheck = (args.no_crashcheck == 'True')
